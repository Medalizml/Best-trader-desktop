...
/**
 * In MVC architectural patterns, a controller operates in a central role with different mechanics. 
 * The controller class is a plain class with some public methods.
 * controller activity is to prepare view data(fxml file )
 * */
...
public class statsCTRL implements Initializable, ControlledScreen {
	...
	/**adding some instance variables that to give access to the table and the labels inside the view. 
	  The fields and some methods have a special @FXML annotation. 
	  This is necessary for the fxml file to have access to those variables. 
	  After we have everything set up in the fxml file, the application will automatically 
	  fill the variables when the fxml file is loaded */
	...
	ObservableList<Customer> data1 = FXCollections.observableArrayList(c.getContext().findAll());
	 
	/** data as an observable list of company.*/
	ObservableList<Company> data = FXCollections.observableArrayList(this.getContext().affiche1());
	
	public OperationServices getContext() {
		OperationServices remote=null;
		try {
			Context context = new InitialContext();
			remote = (OperationServices) context
					.lookup("Blues/OperationServicesImpl!"
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		/**The PropertyValueFactory that we set on the table columns are used to determine which field 
		 * inside the customer objects should be used for the particular column*/
		...
		/**The PropertyValueFactory that we set on the table columns are used to determine which field 
		 * inside the company objects should be used for the particular column*/
		
		compName.setCellValueFactory(new PropertyValueFactory<Company,String>("name"));
	Nature.setCellValueFactory(new PropertyValueFactory<Company,String>("nature"));
	...
	/**extracting the value of benefit from the gain field in the portfolio,
		...
		/**Add observable list data to the table tab3 and data1 to the table data1*/
	...
	/**A Popup dialog is a special window-like container for a scene graph.when this method is invoked,it provides the statistics of 
	 * the customer's benefit as a line graph  */
	public void showpopup()
	{
		Customer c = data1.get(tab0.getSelectionModel().getSelectedIndex());
		ObservableList<Operation> data = FXCollections.observableArrayList(this.getContext().affiche(c.getId()));
		numberofshare.setCellValueFactory(new PropertyValueFactory<Operation,Integer>("numberShares"));
		dateOp.setCellValueFactory(new Callback<CellDataFeatures<Operation, String>, ObservableValue<String>>() {
	        @Override
	        public ObservableValue<String> call(CellDataFeatures<Operation, String> c) {
	            return new SimpleStringProperty(c.getValue().getDateOperation().toString()); }               
		});
		tab1.setItems(data);
	...
	/**A Popup dialog is a special window-like container for a scene graph.when this method is invoked,it provides the statistics of 
	 * the company's yield as a barchart graph  */
	public void showpopup1()
	{
		FXMLLoader loader = new FXMLLoader(statsCTRL.class.getResource("../Screeners/PopUp1.fxml"));
		try{  
		Pane page = (Pane) loader.load();
		    Stage dialogStage = new Stage();
		    dialogStage.setTitle("Details");
	...
	/**gotoscreen methods are available to navigate from screen to another one animated by mouse click event*/
	@FXML
	...
	/*!!!!simulation!!.!*/
	/*public List<Float> ss()
	{
	List<Float> bt= new ArrayList<>();
	
	for(Operation o : this.getContext().affiche()){
		System.out.println(o);
		System.out.println(o.getQuotation());
		System.out.println(o.getPortfolio());
		System.out.println(o.getPortfolio().getCustomer());
		System.out.println( o.getQuotation().getPricehistories().get(o.getQuotation().getPricehistories().size()-1).getPrice());		
		System.out.println("gaint : "+(o.getNumberShares()*o.getQuotation().getPricehistories().get(o.getQuotation().getPricehistories().size()-1).getPrice()-o.getPortfolio().getCapital()));
	
		bt.add(o.getNumberShares()*o.getQuotation().getPricehistories().get(o.getQuotation().getPricehistories().size()-1).getPrice()-o.getPortfolio().getCapital());
		System.out.println("ouiiiiiii"+bt);
	}
	return bt;
	}*/
